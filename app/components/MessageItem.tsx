...
const MessageItem = memo((props: {
  ...
  const ProviderAvatar = useMemo(() => {
    if (allProviderListByKey) {
      return (allProviderListByKey[props.item.providerId]?.providerLogo) ? (
        <Avatar
        ...
        <div className='bg-blue-500 flex mt-1 text-cyan-50 items-center justify-center rounded-full w-8 h-8'>
          {allProviderListByKey[props.item.providerId].providerName.charAt(0)}
        </div>
      );
    } else {
      return <div className='bg-blue-500 flex mt-1 text-cyan-50 items-center justify-center rounded-full w-8 h-8'>Bot</div>
    }
